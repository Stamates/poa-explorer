defmodule Explorer.SmartContract.Verifier do
  ...
  def evaluate_authenticity(_, %{"name" => "", "contract_source_code" => ""}) do
    {:error, %{name: "can't be blank", contract_source_code: "can't be blank"}}
  end

  def evaluate_authenticity(address_hash, %{"name" => name, "contract_source_code" => contract_source_code}) do
    solc_output = CodeCompiler.run(name, contract_source_code)["contracts"][name][name]
